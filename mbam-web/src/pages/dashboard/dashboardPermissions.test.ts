...
describe("dashboard least privilege", () => {
  it("keeps the cashier baseline personal while including assigned-shop products", () => {
    const metrics = getDashboardMetricsForRole("role-cashier");

    expect(metrics).toEqual(["ownSales", "ownTransactions", "products"]);
  ...
  it("allows the cashier product metric only when the API grants product access", () => {
    const cashier = member({
      permissions: ["sale.create", "sale.view", "product.view", "screen.products"],
    });
    expect(getDashboardMetricsForMember(cashier)).toEqual(["ownSales", "ownTransactions", "products"]);
    expect(getDashboardMetricsForMember({ ...cashier, permissions: ["sale.create", "sale.view"] })).toEqual(["ownSales", "ownTransactions"]);
  });

  it("allows only the validated system master baseline when assignable role data omits it", () => {
    const master = member({ roleId: "role-master-owner", scopeLevel: "master", businessId: undefined, businessUnitId: undefined });
    expect(getDashboardMetricsForMember(master)).toEqual(getDashboardMetricsForRole("role-master-owner"));
  ...
  it("keeps a cashier on the personal baseline when an elevated view is requested", () => {
    const cashier = member({ permissions: ["sale.create", "screen.record_transaction"] });
  ...
  it("allows a granted shop view without exposing ungranted shop metrics", () => {
    const seniorCashier = member({ permissions: ["sale.create", "sale.view", "report.view", "screen.transactions"] });
    expect(normalizeDashboardView("shop", seniorCashier)).toBe("shop");
    expect(getDashboardMetricsForMember(seniorCashier, "shop")).toEqual(["unitRevenue", "queued", "pendingCustomers", "transactions"]);
  ...
  it("preserves the baseline encoded in a custom role id", () => {
    const customManager = member({ roleId: "role-custom-member-shop-manager-123", permissions: ["report.view", "screen.reports"] });
