...
function sumPendingPayments(
  records: ReturnType<typeof getScopedPendingPayments>,
): number {
...
export default function MasterDashboard() {
  ...
  const [selectedMetric, setSelectedMetric] =
    useState<DashboardMetricKey | null>(null);
  ...
  const selectedBusiness = findBusiness(
    selectedMember.businessId ?? selectedUnit?.businessId,
  );

  const scopedUnits = useMemo(
    () => getScopedUnits(selectedMember),
    [selectedMember],
  );
  const scopedTransactions = useMemo(
    () => getScopedTransactions(selectedMember, scopedUnits),
    [selectedMember, scopedUnits],
  );
  const scopedPendingPayments = useMemo(
    () => getScopedPendingPayments(selectedMember),
    [selectedMember],
  );
  const scopedBusinessIds = new Set(scopedUnits.map((unit) => unit.businessId));
  const scopedBusinesses = workspace.businesses.filter((business) =>
    scopedBusinessIds.has(business.id),
  );
  const scopedProducts = workspace.products.filter(
    (product) =>
      !product.businessId || scopedBusinessIds.has(product.businessId),
  );
  const scopedTeam = workspace.teamMembers.filter((member) => {
    ...
    if (selectedMember.scopeLevel === "business")
      return member.businessId === selectedMember.businessId;
    return (
      member.businessUnitId === selectedMember.businessUnitId ||
      member.id === selectedMember.id
    );
  });

  const transactionRevenue = sumTransactions(scopedTransactions);
  const unitRevenue = scopedUnits.reduce(
    (sum, unit) => sum + unit.todayRevenue,
    0,
  );
  const displayedRevenue =
    transactionRevenue > 0 ? transactionRevenue : unitRevenue;
  const queuedTransactions = scopedUnits.reduce(
    (sum, unit) => sum + unit.queuedTransactions,
    0,
  );
  ...
  const allMetrics = useMemo<DashboardMetric[]>(
    () => [
      {
        key: "totalRevenue",
        value: formatMoney(displayedRevenue, workspace.masterAccount.currency),
        hintKey: "allUnits",
      },
      {
        key: "businesses",
        value: scopedBusinesses.length,
        hintKey: "masterBusinesses",
      },
      ...
      { key: "team", value: scopedTeam.length, hintKey: "activeTeam" },
      {
        ...
        value: formatMoney(
          pendingOutstanding,
          selectedBusiness?.currency ?? workspace.masterAccount.currency,
        ),
        hintKey: "pendingFollowUp",
      },
      {
        key: "businessRevenue",
        value: formatMoney(
      ...
      {
        key: "transactions",
        value: scopedTransactions.length,
        hintKey: "assignedScope",
      },
      {
        key: "ownSales",
  ...
  const renderTransactions = (records: TransactionRecord[]) => {
    if (records.length === 0) {
      return (
        <p className="card-muted">{t("roleDashboard.labels.noRecords")}</p>
      );
    }
      ...
      <div className="list-stack summary-two-column-list">
        {records.slice(0, 4).map((transaction) => (
          <div className="list-item" key={transaction.id}>
            ...
            <span
              className={
                transaction.status === "queued" ? "badge warning" : "badge"
              }
            >
              {formatMoney(
                transaction.amount,
    ...
    <div className="list-stack summary-two-column-list">
      {scopedUnits.slice(0, 4).map((unit) => (
        <div className="list-item" key={unit.id}>
          ...
          <span
            className={unit.queuedTransactions > 0 ? "badge warning" : "badge"}
          >
            {unit.queuedTransactions > 0
              ? t("dashboard.queuedCount", { count: unit.queuedTransactions })
              : t("dashboard.synced")}
          </span>
        </div>
      ))}
  ...
  const renderRevenue = () => (
    <div className="list-stack summary-two-column-list">
      {scopedUnits.slice(0, 4).map((unit) => {
        const unitTransactions = scopedTransactions.filter(
          (transaction) => transaction.businessUnitId === unit.id,
        );
  ...
  const renderBusinesses = () => (
    <div className="list-stack summary-two-column-list">
      {scopedBusinesses.slice(0, 4).map((business) => {
        const units = scopedUnits.filter(
  ...
  const renderPendingCustomers = () => {
    if (scopedPendingPayments.length === 0) {
      return (
        <p className="card-muted">{t("roleDashboard.labels.noPending")}</p>
      );
    }
          ...
          return (
            <div className="pending-payment-row" key={payment.id}>
              <div className="pending-payment-customer">
                ...
                <small>
                  {unit?.name ?? business?.name ?? t("common.unknownUnit")}
                </small>
              </div>
                ...
                <span>
                  ...
                  <small>
                    {payment.paymentDate
                      ...
                      : t("roleDashboard.labels.noPaymentDate")}
                  </small>
                </span>
              </div>
              <span className="badge warning">
                {formatMoney(
                  ...
                  business?.currency ?? workspace.masterAccount.currency,
                )}
              </span>
            </div>
          );
    ...
    <div className="list-stack summary-two-column-list">
      {scopedTeam.slice(0, 4).map((member) => (
        <div className="list-item" key={member.id}>
          ...
          <span
            className={member.status === "invited" ? "badge warning" : "badge"}
          >
  ...
  const renderDetail = () => {
    switch (activeMetricKey) {
      ...
      default:
        return (
          <p className="card-muted">{t("roleDashboard.detailFallback")}</p>
        );
    }
  };
      ...
      <DevOnly>
        <article className="card role-preview-card">
          <div>
            ...
            <h3>
              {t("roleDashboard.viewingAs")}: {selectedMember.fullName}
            </h3>
            <p className="card-muted">
              {selectedRole
