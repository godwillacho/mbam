...
pub async fn signup(db: &PgPool, config: &Config, payload: SignupRequest) -> Result<AuthResponse, ApiError> {
    let full_name = payload.full_name.trim();
    let email = normalize_email(&payload.email);
    let phone = payload.phone.as_deref().map(str::trim).filter(|value| !value.is_empty());

    validate_signup(full_name, &email, &payload.password)?;

    if repository::find_user_by_email(db, &email).await?.is_some() {
        return Err(ApiError::BadRequest("an account already exists for this email".to_string()));
    }

    let password_hash = password::hash_password(&payload.password).map_err(|_| ApiError::Internal)?;
    let user = repository::create_user_with_master_account(db, full_name, &email, phone, &password_hash).await?;

    build_auth_response(db, config, user.id, user.full_name, user.email, user.email_verified).await
}

/// Authenticates an existing user and creates fresh auth tokens.
pub async fn login(db: &PgPool, config: &Config, payload: LoginRequest) -> Result<AuthResponse, ApiError> {
    let email = normalize_email(&payload.email);
...
async fn build_auth_response(
    ...
    let access_token = tokens::create_access_token(user_id, &config.jwt_access_secret, config.access_token_minutes)
...
fn validate_signup(full_name: &str, email: &str, password: &str) -> Result<(), ApiError> {
    if full_name.len() < 2 {
        return Err(ApiError::BadRequest("full name must be at least 2 characters".to_string()));
    }
