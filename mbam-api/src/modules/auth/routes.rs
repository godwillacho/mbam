...
async fn microsoft_start(State(state): State<AppState>) -> Result<(HeaderMap, Redirect), ApiError> {
    let (Some(client_id), Some(redirect_uri), Some(_client_secret)) = (
...
fn oauth_state_cookie_for(
    ...
    format!(
        "{cookie_name}={value}; HttpOnly; SameSite=Lax; Path=/api/v1/auth/oauth; Max-Age={max_age}{secure}"
    )
}
