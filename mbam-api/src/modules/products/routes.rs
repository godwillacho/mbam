...
use super::{model::{BulkCreateProductsRequest, Product, ProductWriteRequest}, service};
...
async fn list(State(state): State<AppState>, headers: HeaderMap) -> Result<Json<Vec<Product>>, ApiError> {
...
async fn create_bulk(
    ...
    let user_id = state.authentication.authenticate_user_id(&headers, &state.db).await?;
    Ok((StatusCode::CREATED, Json(service::create_bulk(&state.db, user_id, payload).await?)))
}
...
async fn update(
    ...
    let user_id = state.authentication.authenticate_user_id(&headers, &state.db).await?;
    Ok(Json(service::update(&state.db, user_id, product_id, payload).await?))
}
...
async fn disable(
    ...
    let user_id = state.authentication.authenticate_user_id(&headers, &state.db).await?;
    Ok(Json(service::disable(&state.db, user_id, product_id).await?))
}
