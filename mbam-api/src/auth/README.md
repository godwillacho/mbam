...
This directory is the single API boundary for identity-provider
authentication, password/token security primitives, and the legacy
(non-Keycloak) auth provider. It replaces route-level JWT parsing and
provides a controlled migration from Mbam-issued tokens to Keycloak-issued
OpenID Connect access tokens.

Until 2026-07-08 these three concerns lived in three separate top-level
folders (`authentication/`, `security/`, `modules/auth/`), briefly bridged by
a thin re-export facade at this same path. That facade has now been replaced
by physically moving the real files here, so the import path
(`crate::auth::...`) and the folder on disk agree. See "Directory Layout"
below for exactly what moved.
...
or password-reset screens as primary runtime paths. The `legacy/` provider
below still exists and is exercised by `checklist_tests.rs`, but is only
mounted at `/api/v1/auth` when `AUTH_PROVIDER=legacy`.
...
- `mod.rs` -- provider selection (`AuthenticationLayer`) and the public
  `authenticate()`/`authorize()` API every protected route goes through.
...
- `identity_repository.rs` -- subject mapping plus active membership-grant
  loading (renamed from a plain `repository.rs` to stay distinct from
  `legacy/repository.rs`, since both now live under this same tree).
- `password.rs` -- Argon2id password hashing and verification (moved from the
  former `security/` folder).
- `tokens.rs` -- access-token, opaque refresh-token, and signed offline-grant
  issuance/verification (moved from the former `security/` folder).
- `legacy/` -- the non-Keycloak auth provider: HTTP handlers/service/
  repository for signup, login, refresh, logout, OAuth, password reset, and
  offline grants (moved from the former `modules/auth/` folder; see its own
  `legacy/README.md`).
