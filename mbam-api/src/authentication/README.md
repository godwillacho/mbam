...
only when the token is valid and its baseline role agrees with an active local
membership. This prevents accidental privilege expansion if either system is
misconfigured.
...
`resource_access[KEYCLOAK_ROLE_CLIENT_ID].roles`.
...
5. Keycloak baseline roles are compared with active local membership roles.
...
membership, or role mismatch returns `401 Unauthorized` without falling back to
a broader role.
...
- Centralize all protected-route authentication in this directory.
...
a local signing-key cache. It also makes Keycloak availability part of online
API availability. A later optimization may use issuer and audience-validated
JWT verification with cached JWKS plus a short revocation window.
...
- Valid token, linked user, matching active membership: allowed.
...
- Custom Keycloak role without a baseline composite: denied.
