create table if not exists membership_business_scopes (
...
create table if not exists membership_business_unit_scopes (
  membership_id uuid not null references memberships(id) on delete cascade,
...
where product.business_unit_id is null;
...
create unique index if not exists idx_products_unit_sku
...
create index if not exists idx_products_account_unit
  on products (business_account_id, business_unit_id, updated_at);
create index if not exists idx_membership_business_scopes_business
  on membership_business_scopes (business_id, membership_id);
create index if not exists idx_membership_business_unit_scopes_unit
