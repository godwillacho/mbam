...
## 2026-07-01 - Optional Plasmic Visual-Editing Integration

**Related change:** `2026-07-01T19:09:32Z`

**Requested behavior:** Let the user visually rearrange/restyle dashboard UI
components (starting with the metric cards) using Plasmic, without needing to
hand-edit frontend code for layout/styling changes, and without weakening any
authorization or data-loading behavior.

**Root cause / engineering reason:** Not a bug fix — a new optional
integration. The design goal was to make Plasmic control only presentation
(arrangement, styling) of components that are already authorized and already
carrying real data, and to make the integration fully inert (byte-for-byte
identical runtime behavior) until a real Plasmic project is deliberately
configured, since the user does not yet have a Plasmic account or project.

**Files changed:**

- `mbam-web/package.json`, `mbam-web/package-lock.json`
- `mbam-web/src/plasmic-init.ts` (new)
- `mbam-web/src/components/dashboard/MetricCell.tsx` (new; extracted from
  `BaselineDashboards.tsx`)
- `mbam-web/src/components/dashboard/DashboardMetricsGrid.tsx` (new)
- `mbam-web/src/components/dashboard/DashboardMetricsGrid.test.tsx` (new)
- `mbam-web/src/pages/dashboard/BaselineDashboards.tsx`
- `mbam-web/.env.example`
- `docs/plasmic-integration.md` (new)
- `REPOSITORY_MAP.md`
- `debug.log`
- `error.log`
- `docs/ENGINEERING_DEBUG_LOG.md`
...
- Installed `@plasmicapp/loader-react`, a maintained, widely-used package;
  `npm audit` reported 0 vulnerabilities after the install.
- Extracted `MetricCell` (the dashboard card previously defined inline in
  `BaselineDashboards.tsx`) into its own presentational component with no
  data fetching or authorization logic, so it is safe to hand to a visual
  editor.
- Added `src/plasmic-init.ts`: reads `VITE_PLASMIC_PROJECT_ID` and
  `VITE_PLASMIC_PROJECT_TOKEN` from the environment, exposes
  `isPlasmicConfigured()`, and only calls `initPlasmicLoader`/
  `registerComponent` when both are present. With no project configured
  (the current state), `PLASMIC` is `null` and nothing about the app changes.
- Added `DashboardMetricsGrid.tsx` as the integration pattern for any future
  Plasmic-aware component: it always renders the original hardcoded grid
  unless (a) a Plasmic project is configured, and (b) `maybeFetchComponentData`
  confirms a `MbamDashboardMetricsGrid` component actually exists in that
  project. Only then does it render through `PlasmicComponent`/
  `PlasmicRootProvider`, passing the same authorized `definitions`/`summary`/
  `currency` props the hardcoded grid already receives from
  `BaselineDashboards.tsx`. Plasmic never fetches data or makes authorization
  decisions itself.
- Documented the setup, the registration pattern for additional components,
  and the security boundary in `docs/plasmic-integration.md`, including that
  the Plasmic project token is a public loader token (same trust model as
  `VITE_SENTRY_DSN`), not a secret, but should still not be committed with a
  real value.
- Added empty `VITE_PLASMIC_PROJECT_ID`/`VITE_PLASMIC_PROJECT_TOKEN` entries
  to `mbam-web/.env.example` (and to the gitignored local
  `mbam-web/.env.development`).

**Debugging and verification performed:**

- Read the installed package's own `.d.ts` type declarations directly (rather
  than relying on training-data familiarity with the Plasmic API) to confirm
  `maybeFetchComponentData`, `registerComponent`, `PlasmicComponent`, and
  `PlasmicRootProvider` signatures and the `"object"`/`"string"` prop-type
  literals before writing against them.
- `npm run type-check` passed.
- `npm run lint` passed.
- `npm test` passed (52/52 tests), including a new test asserting
  `DashboardMetricsGrid` renders the exact original hardcoded markup when no
  Plasmic project is configured (the only state currently possible, since no
  Plasmic project exists yet).
- `npx vite build --mode production --outDir <temp dir>` succeeded (built to
  an alternate directory; see error log for why this sandbox's real `dist/`
  cleanup step is unrelated and unreliable here).
- `npm audit --audit-level=moderate` reported 0 vulnerabilities.

**Errors encountered:**

- See `error.log` for a cosmetic `eval` build warning originating inside
  Plasmic's own `loader-core` package (not this change's code).

**Checks not run:**

- Backend Rust tests were not rerun; no Rust files changed.
- The actual Plasmic-authored-layout rendering path (`PlasmicComponent`
  successfully rendering a real Studio-authored component) has not been
  exercised end-to-end, because the user does not yet have a Plasmic account
  or project. Only the "not configured" and "configured but component doesn't
  exist yet" fallback paths are currently reachable and tested.
- Live browser verification was not run; no local Docker/Keycloak stack is
  available in this environment.

**Remaining risks and follow-up checks:**

- Once the user creates a Plasmic project and fills in
  `VITE_PLASMIC_PROJECT_ID`/`VITE_PLASMIC_PROJECT_TOKEN`, the end-to-end path
  (build a `MbamDashboardMetricsGrid` component in Plasmic Studio using the
  registered `MbamMetricCell` block, confirm it renders in the running app)
  still needs a first real verification pass.
- If more presentational components are registered with Plasmic later,
  double-check each stays free of data-fetching and authorization logic per
  `docs/security-rules.md`, matching the pattern in `MetricCell.tsx`.
- The production bundle grew (~539 KB main chunk after adding the Plasmic
  loader) and Vite's build now warns about chunk size; worth revisiting
  code-splitting (dynamic `import()` for Plasmic-aware components) if this
  becomes a real page-load concern.
